...
var baseLayerDecor = [
    ...
    18, 20, 18, 20, 18, 20, 18, 20, 18, 20, 18, 20, 18, 20, 18, 20, 18, 20, 18, 20, 18, 20, 18, 20, 18, 20, 18, 20, 18, 20, 18, 20, 18, 20, 18, 20, 18, 20, 18, 20,
    19, 21, 19, 21, 19, 21, 19, 21, 19, 21, 19, 21, 19, 21, 19, 21, 19, 21, 19, 21, 19, 21, 19, 21, 19, 21, 19, 21, 19, 21, 19, 21, 19, 21, 19, 21, 19, 21, 19, 21,
    14, 16, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 14, 16, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 14, 16,
    15, 17, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 15, 17, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 15, 17,
    ...
    6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 15, 17,
    6, 6, 22, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 18, 20,
    6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 22, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 19, 21,
...
function Generator(width, height)
{
    this.width = width;
    this.height = height;
    this.tree = new ContainerTree(new Container(0, 0, Math.floor(width / 4), Math.floor(height / 4)), 2, 0.3, 0.3, 20);
    this.tree.createMap();
    this.mapLayerA = new Array(width * height).fill(2);
    this.mapLayerB = new Array(width * height).fill(2);
}

Generator.prototype.createMap = function(mapKind)
{
    // convert map of 0's and 1's to a map with tiles
    var rawMap = this.tree.map.getMap();
    this.tree.map.printMap();
    var rawMapWidth = this.width / 4;
    var rawMapHeight = this.height / 4;
    for(var i = 0; i < rawMapHeight; ++i)
    {
        for(var j = 0; j < rawMapWidth; ++j)
        {
            if(rawMap[xytoi(j, i, rawMapWidth)] == 0)
            {
                //for(var k = 0; k < 2; ++k)
                {
                        // alternate between layers to create successive rows of trees
                        //if(i % 2 == 0)
                        {
                                this.mapLayerA[xytoi(j * 4 + 0, i * 4 + 0, this.width)] = 14;
                                this.mapLayerA[xytoi(j * 4 + 0, i * 4 + 1, this.width)] = 15;
                                this.mapLayerA[xytoi(j * 4 + 0, i * 4 + 2, this.width)] = 18;
                                this.mapLayerA[xytoi(j * 4 + 0, i * 4 + 3, this.width)] = 19;
                                this.mapLayerA[xytoi(j * 4 + 1, i * 4 + 0, this.width)] = 16;
                                this.mapLayerA[xytoi(j * 4 + 1, i * 4 + 1, this.width)] = 17;
                                this.mapLayerA[xytoi(j * 4 + 1, i * 4 + 2, this.width)] = 20;
                                this.mapLayerA[xytoi(j * 4 + 1, i * 4 + 3, this.width)] = 21;
                                this.mapLayerA[xytoi(j * 4 + 2, i * 4 + 0, this.width)] = 14;
                                this.mapLayerA[xytoi(j * 4 + 2, i * 4 + 1, this.width)] = 15;
                                this.mapLayerA[xytoi(j * 4 + 2, i * 4 + 2, this.width)] = 18;
                                this.mapLayerA[xytoi(j * 4 + 2, i * 4 + 3, this.width)] = 19;
                                this.mapLayerA[xytoi(j * 4 + 3, i * 4 + 0, this.width)] = 16;
                                this.mapLayerA[xytoi(j * 4 + 3, i * 4 + 1, this.width)] = 17;
                                this.mapLayerA[xytoi(j * 4 + 3, i * 4 + 2, this.width)] = 20;
                                this.mapLayerA[xytoi(j * 4 + 3, i * 4 + 3, this.width)] = 21;
                        }
                        //else
                        {
                                this.mapLayerB[xytoi(j * 4 + 0, i * 4 + 0, this.width) + 4 * rawMapWidth] = 14;
                                this.mapLayerB[xytoi(j * 4 + 0, i * 4 + 1, this.width) + 4 * rawMapWidth] = 15;
                                this.mapLayerB[xytoi(j * 4 + 0, i * 4 + 2, this.width) + 4 * rawMapWidth] = 18;
                                this.mapLayerB[xytoi(j * 4 + 0, i * 4 + 3, this.width) + 4 * rawMapWidth] = 19;
                                this.mapLayerB[xytoi(j * 4 + 1, i * 4 + 0, this.width) + 4 * rawMapWidth] = 16;
                                this.mapLayerB[xytoi(j * 4 + 1, i * 4 + 1, this.width) + 4 * rawMapWidth] = 17;
                                this.mapLayerB[xytoi(j * 4 + 1, i * 4 + 2, this.width) + 4 * rawMapWidth] = 20;
                                this.mapLayerB[xytoi(j * 4 + 1, i * 4 + 3, this.width) + 4 * rawMapWidth] = 21;
                                this.mapLayerB[xytoi(j * 4 + 2, i * 4 + 0, this.width) + 4 * rawMapWidth] = 14;
                                this.mapLayerB[xytoi(j * 4 + 2, i * 4 + 1, this.width) + 4 * rawMapWidth] = 15;
                                this.mapLayerB[xytoi(j * 4 + 2, i * 4 + 2, this.width) + 4 * rawMapWidth] = 18;
                                this.mapLayerB[xytoi(j * 4 + 2, i * 4 + 3, this.width) + 4 * rawMapWidth] = 19;
                                this.mapLayerB[xytoi(j * 4 + 3, i * 4 + 0, this.width) + 4 * rawMapWidth] = 16;
                                this.mapLayerB[xytoi(j * 4 + 3, i * 4 + 1, this.width) + 4 * rawMapWidth] = 17;
                                this.mapLayerB[xytoi(j * 4 + 3, i * 4 + 2, this.width) + 4 * rawMapWidth] = 20;
                                this.mapLayerB[xytoi(j * 4 + 3, i * 4 + 3, this.width) + 4 * rawMapWidth] = 21;
                        }
                }
            }
            else
            {
                for(var k = 0; k < 4; ++k)
                {
                    for(var l = 0; l < 2; ++l)
                    {
                        if(randint(0, 50) == 0)
                        {
                            //this.mapLayerA[xytoi(j * 2 + l, i * 4 + k, this.width)] = 22;
                        }

                        if(randint(0, 50) == 0)
                        {
                            //this.mapLayerB[xytoi(j * 2 + l, i * 4 + k, this.width) + 2*this.width] = 22;
                        }
                    }
                }
            }   
    ...
    baseLayerDecor = this.mapLayerA;
    overheadLayerDecor = this.mapLayerB;
}
